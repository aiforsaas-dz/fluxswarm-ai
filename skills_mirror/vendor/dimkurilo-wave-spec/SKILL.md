...
# (variant/effort НЕ переключается из агента — host-side; см. model-card.md §LAUNCH РЕЦЕПТ)
...
- "Prohibited" section (vv-controller, terminal send, variant-switch attempt, result.id, Qwen without yolo, launch without model-card check, same-family review, coordinator writes code)
...
5. **No copy-paste bash blocks:** the orchestrator builds dispatch commands from the orchestration guide itself. NEXT_SESSION provides the SEQUENCE (create → wait → task-create → dispatch → check) but NOT the exact bash.
...
- Step 6: Commit + push + PR (merge — владелец; шаблон NEXT_SESSION_ITER.md.tmpl §6)
...
| Security / fidelity gate (optional) | **GPT-5.5** (`codex` CLI) — opt-in for highly sensitive cases; NOT default Strong pair | GPT-5.5 (historical unique role, retained as opt-in) |
...
| **Simple** | 1 | **yes** (`writer.family ≠ reviewer.family`) | default (variant НЕ переключается — host-side) | Low-risk change that still warrants one independent look |
...
**Effort is NOT switched from the agent.** Every mode runs at the launch default (DeepSeek default, GLM max, Qwen high/max). The old "Simple = `/variants low`" contract is removed — the agent has no tool to switch variant (host-side only). See `multi-model-orchestration/references/model-card.md` §LAUNCH РЕЦЕПТ.
